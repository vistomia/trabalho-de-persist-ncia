...
async def read_operators(
    ...
    """List operators with pagination and optional filters"""
...
async def read_operator_by_ids(
    ...
    """Get operator by server_id and user_id (composite primary key)"""
...
async def read_operators_ordered(
    ...
    """List operators with ordering and pagination"""
    query = select(Operator)
...
async def create_operator(operator: Operator, session: Session = Depends(get_session)):
    """Create a new operator relationship"""
    # Validate that the server and user exist
...
async def update_operator(
    ...
    for key, value in operator_update.items():
        if key in allowed_fields and hasattr(operator, key) and value is not None:
...
async def delete_operator(
    ...
    """Delete operator relationship by server_id and user_id"""
    operator = session.exec(
        select(Operator).where(
            and_(Operator.server_id == server_id, Operator.user_id == user_id)
        )
...
async def delete_operators_by_server(server_id: int, session: Session = Depends(get_session)):
    """Delete all operator relationships for a server"""
